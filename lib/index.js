...
manager.prototype.init = function(cb) {
	...
	var cwd = NWJS.App.dataPath; //process.cwd();
	...
	this.Project.paths = {
			...
			npm : Path.join(this.__dir, '/node_modules/npm/bin/npm-cli.js')
	}
...
manager.prototype.log = function() {
		...
		this.snowlog.info.apply(this,Array.prototype.slice.call(arguments));
}
...
manager.prototype.getDoc = function(id,cb) {
	...
	_this.db.config.findOne({ _id: id }, function (err, doc) {
		...
		if(_this.type === 'module') {
			_this.getView(callback);
		} else if(_this.type === 'branch') {
			_this.getPackageFromGitHub(_this.version,callback);
		} else {
			_this.type = 'module';
			_this.getView(callback);
		}
...
manager.prototype.getPackageFromGitHub = function(val,cb) {
	...
	this.GitHub.repos.getContent(opts, function(err, res) {
		...
		if(err) {
			...
			Manager.log('package from github error',err,'Retry? ',_this._gitPackageTries,_this._gitPackageTries < 4);
		...
		if(res.download_url) {
			request.get(res.download_url, function(res){
				//Manager.log('request get',res.text,res.body);
				try {
				...
				//Manager.log(_this.current[val],res.text)
...
manager.prototype.build = function(opts) {
	...
	async.series([
		...
		function(next) {
			...
			if(!path)path = _this.Project.paths.root;
			if(path.charAt(0) !== '/')path =_this.Project.paths.root + '/' + path;
		...
		function(next) {
			...
			var finalRun = function() {
				...
				_.each(_this.modules,function(module) {
					pkg.dependencies[module.module] = module.version || 'latest';
				});
				...
				jf.writeFile(file, pkg, function(err) {
					if(err) {
						...
						_build.options.dependencies = pkg.dependencies;
						_build.pkg = pkg;
		...
		function(next) {
			...
			if(os.platform().charAt(0).toLowerCase() === 'w') {
				_this.emit('build',{action:'gzip',success:false,err:'Windows is either not zipping or saving the zip file for me.  Please help if you can'});
				return next();
			}
		...
		function(next) {
			...
			var npmCommand = _this.Project.paths.npm ? _this.Project.paths.npm  : 'npm';
			
			if(_this.version === 'link') {
					_this.emit('build',{action:'npm',working:'Running npm link ' + _this.Project.npm});					
					_this.log('Linking project using npm link');
					npm.prefix = _build.path;
					npm.load(npm.config, function (err) {
					  // catch errors
						if(err) {
							_this.emit('build',{action:'npm',success:false,err:'Error: ' +err});
							return _this.log('ERROR LOADING NPM',err);
							
						}
						npm.commands.link(_this.Project.npm, function (er, data) {
							_this.log('running npm link ' + _this.Project.npm,er,data);
							if(data) {
								_this.emit('build',{action:'npm',working:'Linked  ' + data[0] + ' to ' + data[2]});
								install();
							}
						});
					...
					npm.load(npm.config, function (err) {
					  // catch errors
						if(err) {
							...
							_this.log('ERROR LOADING NPM',err);
							return next();
						}
						npm.commands.install(_build.path, [], function (er, data) {
							_this.log('running npm install',er,data);
							
							if(data) {
								_this.emit('build',{action:'npm',working:data});
								_this.emit('build',{action:'npm',success:data});
								next();
							}
						});
						_tmp.loginstall = function (message) {
							// log the progress of the installation
							_this.emit('build',{action:'npm',working:message.toString()});
							_this.snowlog.info(chunk.toString());
						}
						npm.on("log", _tmp.loginstall);
						npm.on("message", _tmp.loginstall);
						npmlog.on("log", _tmp.loginstall);
					});
	...
	var start = function(doc) {
		...
		try {
			...
			child.on('start', function(process,data) {
				...
				_this.updateDocById(doc._id,{pid:child.child.pid,status:'running',build:_build},function() {
					_this.emit('node',{server:doc._id,log:'Server Starting'});
					Manager.App.qFlash('loadingmessage','Server Starting - pid: ' + child.child.pid,4000);
				});
...
manager.prototype.removeNodeBuildDir = function(id,cb) {
	...
	var updateMainDoc = function(callback) {
		
		if(id === _this.doc.id) {
			console.log('update doc and return from delete build');
			return _this.getDoc(id,callback);
		}
		console.log('return from delete build');
		callback();
	}
	this.getDocById(id,function(err,doc) {
		if(err || !doc) {
...
manager.prototype.openWindow = function(id,file) {
	if(!file)Manager.log('no file to open window for');
	
	var win = NWJS.Window.open(file, {
	  position:'center',
	  width:1200,
	  height:800,
	  focus:true
