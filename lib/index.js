...
manager.prototype.init = function(cb) {
	...
	var cwd = process.cwd();
	this.Project.paths = {
			gzip: Path.join(NWJS.App.dataPath, _this.Project.name, 'builds', 'archives'),
			pid: Path.join(NWJS.App.dataPath, _this.Project.name, 'pid'),
			logs: Path.join(cwd, 'logs'),
			dbConfig:  Path.join(NWJS.App.dataPath, _this.Project.name),
			dbBuild: Path.join(NWJS.App.dataPath, _this.Project.name, 'builds', 'db')
	}
...
manager.prototype.build = function(opts) {
	...
	async.series([
		...
		function(next) {
			...
			_build.gzipName = doc.saneName + '.' + moment(new Date()).format() +'.tar.gz';
